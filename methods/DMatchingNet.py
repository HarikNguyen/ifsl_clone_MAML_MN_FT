...
class DMatchingNet(MetaTemplate):
    def __init__(
        ...
        if self.classifier == "single":
            ...
            self.matching_nets = nn.ModuleList(
                [MatchingNetModule(feat_dim).cuda() for i in range(n_splits)]
            )
            ...
            self.x_matching_nets = nn.ModuleList(
                [MatchingNetModule(x_feat_dim).cuda() for i in range(n_splits)]
            )
            self.d_matching_nets = nn.ModuleList(
                [MatchingNetModule(d_feat_dim).cuda() for i in range(n_splits)]
            )
    ...
    def set_forward(self, x, is_feature=False):
        ...
        for i in range(self.n_splits):
            if self.classifier == "single":
                support_new, query_new = self.matching_nets[i](
                    fused_support[i], fused_query[i]
                )
                _, c_query_new = self.matching_nets[i](
                    fused_support[i], c_fused_query[i]
                )
                scores[i] = (
                    self.relu(
                        self.normalize(query_new).mm(
                            self.normalize(support_new).transpose(0, 1)
                        )
                    )
                    * self.temp
                )
                ...
                support_x_new, query_x_new = self.x_matching_nets[i](
                    split_support[i], split_query[i]
                )
                support_d_new, query_d_new = self.d_matching_nets[i](
                    support_d[i], query_d[i]
                )
                x_score = self.relu(
                    self.normalize(query_x_new).mm(
                        self.normalize(support_x_new).transpose(0, 1)
                    )
                )
                d_score = self.relu(
                    self.normalize(query_d_new).mm(
                        self.normalize(support_d_new).transpose(0, 1)
                    )
                )
