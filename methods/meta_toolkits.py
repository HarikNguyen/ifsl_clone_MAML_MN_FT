...
class FeatureProcessor:
    def __init__(
        ...
        print(
            self.preprocess_before_split,
            self.preprocess_after_split,
            self.normalize_before_center,
            self.normalize_d,
            self.normalize_ed,
        )

        pretrain_features = self.pretrain.get_pretrained_class_mean(
            normalize=is_cosine_feature
        )
        self.pretrain_features = (
            torch.from_numpy(pretrain_features).float().cuda()[:num_classes]
        )
        if normalize_d:
    ...
    def get_features(self, support, query):
        ...
        split_support = self.get_split_features(
            support,
            preprocess=True,
            center=pmean_support,
            preprocess_method=self.preprocess_after_split,
        )
        split_query = self.get_split_features(
            query,
            preprocess=True,
            center=pmean_query,
            preprocess_method=self.preprocess_after_split,
...
class MAMLBlock(nn.Module):
    def __init__(self, feat_dim, n_way, update_step, approx=True, lr=0.01):
        ...
        self.feature = nn.Sequential(
            backbone.Linear_fw(self.in_dim, self.hidden_dim),
            nn.ReLU(),
...
class LEOBlock(nn.Module):
    ...
    def prepare_for_rel_net(self, embeddings):
        ...
        interleave_emb = torch.repeat_interleave(
            embeddings, support_size, dim=0
        )  # 000001111122222...
    ...
    def forward_relation_net(self, embeddings):
        ...
        outputs = relation_output.view(
            self.n_way, -1, support_size, self.relation_dim
        )  # 5way*5shot*25*128
    ...
    def possibly_sample(self, distribution_params, stddev_offset=0.0):
        ...
        stddev = stddev - Variable(
            torch.tensor(1.0 - stddev_offset).cuda().double().expand(stddev.size())
        )
    ...
    def forward_decoder(self, latents):
        ...
        fan_out = self.n_way
        stddev_offset = math.sqrt(2.0 / (fan_out + fan_in))
    ...
    def calc_kl_penalty(self, latent_samples, latent_distributions):
        ...
        kl = (
            latent_distributions.log_prob(latent_samples)
            - prior.log_prob(latent_samples)
        ).mean()
        self.kl_penalty = kl
    ...
    def calc_orthogonality_penalty(self):
        ...
        orthogonality_penalty = (
            (corr_matrix - identity) * (corr_matrix - identity)
        ).mean()
