...
def save_features(model, data_loader, outfile, params):
    ...
    count_var[0] = count
    np.save(
        "img_paths_%s_%s_%s.npy" % (params.dataset, params.model, params.method),
        all_paths,
    )
    f.close()
...
def baseline_s2m2_init(params):
    assert (
        params.method != "maml" and params.method != "maml_approx"
...
def simple_shot_init(params, split):
    ...
    if params.dataset == "tiered":
        num_classes = 351
    model = simple_shot_models.__dict__[model_name](
        num_classes=num_classes, remove_linear=True
    )
    model = model.cuda()
    ...
    data_loader = datamgr.get_data_loader(
        loadfile, aug=False, num_workers=12, tiered_mini=tiered_mini
    )
...
def cosine_init(params, split):
    ...
    if params.dataset == "cross" or params.dataset == "miniImagenet":
        num_classes = 64
        model_dir = os.path.join(
            configs.cosine_dir, "miniImagenet", model_name, "max_acc.pth"
        )
    elif params.dataset == "tiered":
        num_classes = 351
        model_dir = os.path.join(
            configs.cosine_dir, "tiered", model_name, "max_acc.pth"
        )
